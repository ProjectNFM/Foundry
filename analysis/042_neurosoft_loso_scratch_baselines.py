...
def plot_loso_per_subject(df: pd.DataFrame) -> Path:
    ...
    fig, axes = plt.subplots(
        1, n_species, figsize=(7 * n_species, 5), squeeze=False
    )
    ax_idx = 0
    ...
    for species in species_list:
        ...
        ax.set_xticks(x)
        ax.set_xticklabels(
            subset["subject"].values, fontsize=9, rotation=45, ha="right"
        )
...
def main() -> None:
    ...
    for species in ["minipigs", "monkeys"]:
        ...
        print(f"{'─' * 70}")
        print(
            subset[["subject", "best_f1", "run_name", "state"]].to_string(
                index=False
            )
        )
    ...
    print(
        summary[["species", "mean±std", "min", "max", "count"]].to_string(
            index=False
        )
    )

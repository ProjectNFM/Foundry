"""Brain Invaders P300 from-scratch baselines (exp 024).

Compares 5 model conditions × 3 folds for binary P300 classification.

WandB project: foundry_finetuning
Group: BI_P300_BASELINES
...
def parse_run_name(name: str) -> tuple[str, int] | None:
    ...
    Expected patterns:
      bi_p300_eegnet_fold0
      bi_p300_per_channel_cwt_cnn_ch-disabled_fold1
      bi_p300_per_channel_resample_cnn_ch-dynamic_fold2
    """
    if "eegnet" in name:
        for f in FOLDS:
            if f"fold{f}" in name:
...
def print_summary(data: dict) -> None:
    ...
    print(f"\n{'=' * 90}")
    print("  Brain Invaders P300 From-Scratch Baselines")
    print(f"{'=' * 90}")
...
def plot_main_results(data: dict) -> None:
    ...
    ax.set_title(
        "Brain Invaders P300 Baselines: All Conditions",
        fontsize=13,
        fontweight="bold",
    )
    ax.axhline(
    ...
    plt.tight_layout()
    out = FIGURES_DIR / "024_bi_p300_main_results.png"
...
def plot_f1_curves(data: dict) -> None:
    ...
    ax.set_title(
        "Brain Invaders P300: Validation F1 Learning Curves (fold 0)",
        fontsize=13,
        fontweight="bold",
    )
    ...
    plt.tight_layout()
    out = FIGURES_DIR / "024_bi_p300_f1_curves.png"
    fig.savefig(out, dpi=150, bbox_inches="tight")
    print(f"  Saved: {out}")
    plt.close()
...
def plot_fold_variance(data: dict) -> None:
    ...
    ax.set_title(
        "Brain Invaders P300: Cross-Fold Variance",
        fontsize=13,
        fontweight="bold",
    )
    ...
    plt.tight_layout()
    out = FIGURES_DIR / "024_bi_p300_fold_variance.png"
    fig.savefig(out, dpi=150, bbox_inches="tight")
    print(f"  Saved: {out}")
    plt.close()
...
def main():
    print("=" * 70)
    print("  Brain Invaders P300 From-Scratch Baselines Analysis")
    print("=" * 70)

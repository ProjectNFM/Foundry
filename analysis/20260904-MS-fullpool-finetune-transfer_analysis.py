...
Usage (corrected matched-LR matrix):
    uv run python analysis/20260904-MS-fullpool-finetune-transfer_analysis.py

Usage (invalidated 2.5e-4 downstream matrix):
    PHASE4A_TRANSFER_CONDITION=invalid_lr2p5e4 \
      uv run python analysis/20260904-MS-fullpool-finetune-transfer_analysis.py
"""
...
CELL_STEM = (
    "phase4a-downstream-lr1p5e3"
    if TRANSFER_CONDITION == "corrected_lr1p5e3"
    else "phase4a-downstream"
)
CELL_LISTS = {
    species: ROOT / f"launch/phase4a/{CELL_STEM}-{species}.jsonl"
    for species in ("minipigs", "monkeys")
}
LOCAL_RUN_ROOT = Path(
    "/network/scratch/s/sobralm/runs/PHASE4A_FULL_FINETUNE_MINIPIGS"
)
SOURCE_GROUPS = {
...
RECOVERED_TRANSFER_RUN_IDS = FAILED_RESTART_RECOVERY_IDS | frozenset(
    {"91cb2c2e"}
)
EXPECTED_RECOVERY_IDS = (
    frozenset()
    if TRANSFER_CONDITION == "corrected_lr1p5e3"
    else RECOVERED_TRANSFER_RUN_IDS
)
RUN_COLUMNS = [
    "kind",
    "species",
...
def is_declared_transfer_run(
    ...
    observed = {
        "run_name": run_name,
        "species": species,
        "target_recording": str(recording_ids[0])
...
def recovered_local_summary(
    ...
    for path in (LOCAL_RUN_ROOT / run_name / "wandb").glob(
        ...
        try:
            ...
            raise RuntimeError(
                f"Could not read recovery summary {path}"
            ) from exc
        values = [
    ...
    if len(candidates) != 1:
        raise RuntimeError(
            f"Expected one complete first-attempt summary for {run_id}; found {len(candidates)}."
        )
...
def fetch_group(
    ...
    for run in api.runs(
        ...
        if label == "transfer" and not is_declared_transfer_run(
            run_id, name, config, species, selected_cells
        ):
            continue
        ...
        rows.append(
            {
                ...
                "checkpoint_id": nested(config, "run", "checkpoint_id"),
                "state": run.state,
                "analysis_usable": label != "transfer"
                or run.state == "finished"
                or run_id in RECOVERED_TRANSFER_RUN_IDS,
                "metric_source": metric_source,
                "recovery_summary_path": recovery_path,
                "created_at": getattr(run, "created_at", None),
                "recording": record,
                "subject": subject_id(record, config),
                "target_fraction": training_fraction(config, name),
                "target_seed": int(target_seed)
                if target_seed is not None
                else None,
                "source_seed": selected["source_selection_seed"]
                if selected
                else source_seed(config),
                "source_model_seed": selected["source_model_seed"]
                if selected
                ...
                "best_wall_time_s": summary_scalar(
                    summary, "compute/best_wall_time_s"
                ),
            }
        )
    return rows
...
def completion_audit(
    ...
    for label, frame, expected in (
        ...
        ("full finetune", transfer, EXPECTED_TRANSFER),
    ):
        counts = (
            frame.groupby(["species", "state"]).size().unstack(fill_value=0)
        )
        print(f"{label} W&B states:\n{counts.to_string()}")
        for species, n_expected in expected.items():
            if len(frame[frame.species == species]) != n_expected:
                raise RuntimeError(
                    f"{species} {label}: expected {n_expected} declared cells."
                )
    ...
    if set(transfer.cell_id.dropna().astype(str)) != set(selected_cells):
        raise RuntimeError(
            "Declared transfer cells do not exactly match W&B provenance."
        )
    recovered = transfer[
        transfer.metric_source.str.startswith("verified_local_")
    ].copy()
    if set(recovered.run_id) != EXPECTED_RECOVERY_IDS:
        raise RuntimeError("Unexpected local-result recovery set.")
    failed_restart = recovered[
        recovered.run_id.isin(FAILED_RESTART_RECOVERY_IDS)
    ]
    if not failed_restart.state.eq("failed").all():
        raise RuntimeError(
            "The four preemption/ECC recoveries must retain failed W&B states."
        )
    if (
        "91cb2c2e" in EXPECTED_RECOVERY_IDS
        and not recovered.loc[recovered.run_id.eq("91cb2c2e"), "state"]
        .eq("finished")
        .all()
    ):
        raise RuntimeError(
    ...
    return recovered[
        ["run_id", "cell_id", "state", "metric_source", "recovery_summary_path"]
        + REQUIRED_TRANSFER_METRICS
    ]
...
def canonical_scratch(scratch: pd.DataFrame) -> pd.DataFrame:
    ...
    if full.duplicated(unit, keep=False).any():
        raise RuntimeError(
            "100%-data scratch controls are not unique by session/seed."
        )
...
def validation_epoch_to_80pct(
    ...
    if "epoch" not in history or VALIDATION_METRIC not in history:
        raise RuntimeError(
            f"{run_id}: missing validation epoch/history metric."
        )
    history = (
        history[["epoch", VALIDATION_METRIC]].dropna().sort_values("epoch")
    )
    if history.empty:
    ...
    if reached.empty:
        raise RuntimeError(
            f"{run_id}: never reached its 80% validation threshold."
        )
...
def add_validation_convergence(
    ...
    cache_path = (
        csv_dir(__file__)
        / f"{PREFIX}_{label.replace(' ', '_')}_validation_history.csv"
    )
    if cache_path.exists():
        try:
            ...
            cached = pd.DataFrame(
                columns=[
                    "run_id",
                    "validation_epoch_to_80pct",
                    "validation_max_f1_history",
                    "validation_records",
                ]
            )
        if cached.run_id.duplicated().any():
            raise RuntimeError(
                f"Duplicate run IDs in validation-history cache {cache_path}."
            )
    ...
    pending = [
        str(run_id) for run_id in runs.run_id if str(run_id) not in existing
    ]
    total = len(runs)
...
def paired_effects(
    ...
    transfer = transfer[
        transfer.analysis_usable
        & transfer.transfer_regime.eq("full_finetuning")
    ...
    pretrained = transfer.groupby(unit, as_index=False).agg(
        ...
        pretrained_validation_epoch_to_80pct=(
            "validation_epoch_to_80pct",
            "mean",
        ),
    )
    ...
    for metric in COMPUTE_METRICS:
        ...
        paired[f"{metric}_saved_pct"] = (
            100 * paired[f"{metric}_saved"] / paired[f"scratch_{metric}"]
        )
...
def reductions(paired: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    subject = session.groupby(["species", "subject"], as_index=False).mean(
        numeric_only=True
    )
...
def bootstrap_interval(values: pd.Series) -> tuple[float, float]:
    ...
    means = rng.choice(data, size=(20_000, len(data)), replace=True).mean(
        axis=1
    )
...
def species_summary(
    paired: pd.DataFrame, session: pd.DataFrame, subject: pd.DataFrame
) -> pd.DataFrame:
    rows: list[dict[str, Any]] = []
    for species, data in subject.groupby("species"):
        ...
        row: dict[str, Any] = {
            ...
            "test_f1_gain_signflip_p_one_sided": sign_flip_p_positive(
                data.test_f1_gain
            ),
        }
        for metric in COMPUTE_METRICS:
        ...
        row["validation_epoch_to_80pct_saved"] = (
            data.validation_epoch_to_80pct_saved.mean()
        )
        row["validation_epoch_to_80pct_saved_sd_subject"] = (
            data.validation_epoch_to_80pct_saved.std(ddof=1)
        )
        row["validation_epoch_to_80pct_saved_bootstrap_95_low"] = (
            convergence_low
        )
        row["validation_epoch_to_80pct_saved_bootstrap_95_high"] = (
            convergence_high
        )
        row["validation_epoch_to_80pct_saved_signflip_p_one_sided"] = (
            sign_flip_p_positive(data.validation_epoch_to_80pct_saved)
        )
        row["validation_epoch_to_80pct_saved_pct"] = (
            data.validation_epoch_to_80pct_saved_pct.mean()
        )
        rows.append(row)
...
def plot_subject_f1(subject: pd.DataFrame, output: Path) -> None:
    ...
    for axis, species in zip(axes, ("minipigs", "monkeys"), strict=True):
        ...
        axis.set(
            title=species.title(),
            yticks=y,
            yticklabels=data.subject,
            xlabel="Transfer − scratch test macro-F1",
        )
        axis.grid(axis="x", alpha=0.25)
    axes[0].set_ylabel("Subject (session- and seed-averaged)")
    axes[1].legend(loc="best", fontsize=8)
    fig.suptitle(
        "Paired subject-balanced test effect (positive favors transfer)"
    )
    fig.tight_layout()
...
def plot_session_f1(session: pd.DataFrame, output: Path) -> None:
    ...
    for axis, species in zip(axes, ("minipigs", "monkeys"), strict=True):
        ...
        axis.scatter(
            data.scratch_test_supported_f1,
            data.pretrained_test_supported_f1,
            color="#009e73",
            s=35,
            alpha=0.85,
        )
...
def plot_compute_savings(subject: pd.DataFrame, output: Path) -> None:
    ...
    for axis, metric in zip(axes.flat, COMPUTE_METRICS, strict=True):
        for x, species, color in (
            (0, "minipigs", "#0072b2"),
            (1, "monkeys", "#d55e00"),
        ):
            data = subject[subject.species.eq(species)][f"{metric}_saved_pct"]
            offsets = (
                np.linspace(-0.09, 0.09, len(data))
                if len(data) > 1
                else np.array([0.0])
            )
            axis.scatter(
                np.full(len(data), x) + offsets,
                data,
                color=color,
                s=36,
                alpha=0.9,
            ...
            axis.scatter(
                [x], [data.mean()], color="black", marker="D", s=42, zorder=3
            )
        axis.axhline(0, color="black", linewidth=0.8)
        axis.set(
            title=labels[metric],
            xticks=[0, 1],
            xticklabels=["Minipigs", "Monkeys"],
        )
...
def plot_validation_convergence(subject: pd.DataFrame, output: Path) -> None:
    ...
    for axis, species in zip(axes, ("minipigs", "monkeys"), strict=True):
        ...
        axis.set(
            title=species.title(),
            xlabel="Scratch validation epochs to 80% peak",
        )
        axis.grid(alpha=0.25)
    axes[0].set_ylabel("Transfer validation epochs to 80% of own peak")
    fig.suptitle(
        "Subject means; points below the diagonal favor faster transfer"
    )
    fig.tight_layout()
    fig.savefig(output, dpi=180, bbox_inches="tight")
    plt.close(fig)
...
def main() -> None:
    ...
    scratch_full = add_validation_convergence(
        api, entity, scratch_full, "scratch 100%"
    )
    ...
    tables = {
        ...
        "transfer": transfer,
        "scratch_raw": scratch,
        "recovered_first_attempts": recovery,
        "scratch_100pct_canonical": scratch_full,
        "paired": paired,
    ...
    columns = [
        ...
        "n_subjects",
        "n_sessions",
        "n_session_seed_pairs",
        "scratch_test_supported_f1",
        "pretrained_test_supported_f1",
        "test_f1_gain",
        "test_f1_gain_bootstrap_95_low",
        "test_f1_gain_bootstrap_95_high",
        "test_f1_gain_signflip_p_one_sided",
        "best_step_saved_pct",

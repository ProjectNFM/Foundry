"""Session embedding mode comparison — intrasession sanity check (exp 015).

Compares train-val loss curves and best validation performance across three
session embedding modes (static, disabled, dynamic) under intrasession
splitting.  Mirrors the analysis from exp 014 but expects reversed ranking.
...
def print_summary(data: dict) -> None:
    print(f"\n{'=' * 90}")
    print("  Session Embedding Modes — Intrasession (exp 015)")
    print(f"{'=' * 90}")
...
def plot_val_comparison(data: dict) -> None:
    ...
    plt.suptitle(
        "Session Embedding Modes — Intrasession (exp 015)",
        fontsize=14,
        fontweight="bold",
        y=1.02,
    )
    plt.tight_layout()
    out = FIGURES_DIR / "015_val_comparison.png"
...
def plot_learning_curves(data: dict) -> None:
    """Train/val loss curves overlaid per mode."""
    modes = ["Static", "Disabled", "Dynamic"]
    ...
    plt.suptitle(
        "Learning Curves — Session Embedding Modes (Intrasession)",
        fontsize=14,
        fontweight="bold",
        y=1.02,
    )
    plt.tight_layout()
    out = FIGURES_DIR / "015_learning_curves.png"
    fig.savefig(out, dpi=150, bbox_inches="tight")
    print(f"Figure saved: {out}")
    plt.close()
...
def plot_val_overlay(data: dict) -> None:
    """Overlay val loss curves for all three modes."""
    modes = ["Static", "Disabled", "Dynamic"]
    ...
    ax.set_title(
        "Validation Loss — Session Embedding Modes (Intrasession)",
        fontsize=14,
        fontweight="bold",
    )
    ...
    plt.tight_layout()
    out = FIGURES_DIR / "015_val_overlay.png"
    fig.savefig(out, dpi=150, bbox_inches="tight")
    print(f"Figure saved: {out}")
    plt.close()
...
def main():
    print("Fetching runs for session embedding intrasession check (exp 015)...")
    data = fetch_all_data()
    print_summary(data)

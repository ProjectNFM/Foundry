...
def overfit(model, batch, n_steps=500, lr=0.001, label=""):
            ...
            adapter_gnorm = (
                np.mean(
                    [
                        ...
                        if "session_adapter" in n
                        and "weight" in n
                        and p.grad is not None
                    ]
                )
                if any(
                    "session_adapter" in n and "weight" in n
                    for n, _ in model.named_parameters()
                )
                else 0
            )
            print(
                f"  Step {step:4d}: loss={loss.item():.4f} acc={acc:.3f} "
    ...
    print(
        f"  -> {'PASS' if converged else 'FAIL'} final_loss={loss.item():.4f} acc={acc:.3f}"
    )
    return converged
...
class ScaledBiGRU(NeurosoftConvBiGRU):
    ...
    def forward(self, *, input_values, **kwargs):
        return super().forward(
            input_values=input_values * self.scale_factor, **kwargs
        )
...
class NoBiasAdapterBiGRU(nn.Module):
    ...
    def __init__(self, *, task_configs, session_configs, **kwargs):
        ...
        self.channel_counts = {
            str(k): int(v) for k, v in session_configs.items()
        }
    ...
    def forward(
        ...
        **_,
        ...
        if input_seq_len is None:
            seq_lens = torch.full(
                (B,), T_pad, dtype=torch.long, device=input_values.device
            )
        else:
            seq_lens = torch.as_tensor(
                input_seq_len, dtype=torch.long, device=input_values.device
            )

        if input_channel_counts is None:
            input_channel_counts = [self.channel_counts[s] for s in session_ids]
        ch_counts = torch.as_tensor(
        ...
        for i in range(B):
            ...
            c = int(ch_counts[i])
            t = int(seq_lens[i])
            adapted = self.session_adapter[sid](
                input_values[i, :c].transpose(0, 1)
            ).transpose(0, 1)
            adapted[:, t:] = 0
            out[i] = adapted
        ...
        embedding = (x * mask.unsqueeze(-1)).sum(
            dim=1
        ) / feature_lengths.unsqueeze(1).to(x.dtype)
...
class ZNormBiGRU(NeurosoftConvBiGRU):
    ...
    def __init__(self, channel_means, channel_stds, **kwargs):
        ...
        self.register_buffer(
            "channel_means",
            torch.tensor(channel_means, dtype=torch.float32).view(1, -1, 1),
        )
        self.register_buffer(
            "channel_stds",
            torch.tensor(channel_stds, dtype=torch.float32).view(1, -1, 1),
        )
    ...
    def forward(self, *, input_values, **kwargs):
        ...
        normalized[:, :C, :] = (input_values[:, :C, :] - self.channel_means) / (
            self.channel_stds + 1e-8
        )
...
def check_monkey_signal_scale():
    ...
    ds_mp = NeurosoftMinipigs2026(
        ...
        recording_ids=[MINIPIG_RID],
    )
    ds_mk = NeurosoftMonkeys2026(
        root=DATA_ROOT,
        split_type="intrasession-causal",
        task_type="acoustic_stim",
        recording_ids=[MONKEY_RID],
    )

    for name, ds, rid in [
        ("Minipig", ds_mp, MINIPIG_RID),
        ("Monkey", ds_mk, MONKEY_RID),
    ]:
        rec = ds.get_recording(rid)
        signal_source = (
            rec.ecog
        ...
        ch_types = (
            rec.channels.type.astype(str)
            if hasattr(rec.channels, "type")
            else None
        )
        supported = {"eeg", "ecog", "seeg", "ieeg"}
        if ch_types is not None:
        ...
        print(
            f"    Per-channel std: min={signal.std(axis=0).min():.8f}, "
...
def run_full_training_test(
    model, ds, rid, task_config, n_epochs=50, lr=0.0015, label=""
):
    ...
    def make_loader(split):
        ...
        filtered_intervals = {
            rid: filter_intervals_by_mapping(
                intervals[rid], class_mapping, "behavior_labels"
            )
        }
    ...
    for epoch in range(n_epochs):
        ...
        for batch in train_loader:
            ...
            with torch.no_grad():
                ...
                preds = logits.argmax(dim=-1)
                train_correct += (
                    (preds[valid_mask] == targets[valid_mask]).sum().item()
                )
...
def main():
    ...
    batch, raw_items = get_balanced_batch(
        ds, MINIPIG_RID, ref_model, task_config, n_per_class=2
    )
    ...
    with torch.no_grad():
        ...
        for k, v in batch.items():
            batch_dev[k] = v.to(DEVICE) if isinstance(v, torch.Tensor) else v
        batch_dev.pop("target_values")
        batch_dev.pop("target_weights")
        batch_dev.pop("session_id")
        batch_dev.pop("absolute_start")
        input_vals = batch_dev["input_values"] * 1e4
        print(
            f"  Scaled input: range=[{input_vals.min():.4f}, {input_vals.max():.4f}], "

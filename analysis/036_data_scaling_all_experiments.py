...
PRETRAIN_RUNS = {
    "A1": {"name": "pretrain_A1_klinzing_small", "group": "DATA_SCALING_VOLUME",
            "label": "A1: Klinzing small (28 rec)", "eff_data": 2338, "n_datasets": 1,
            "experiment": "volume"},
    "A2": {"name": "pretrain_A2_klinzing_full", "group": "DATA_SCALING_VOLUME",
            "label": "A2: Klinzing full (256 rec)", "eff_data": 19484, "n_datasets": 1,
            "experiment": "volume"},
    "A3": {"name": "pretrain_A3_shirazi_only", "group": "DATA_SCALING_VOLUME",
            "label": "A3: Shirazi (1342 rec, 129ch)", "eff_data": 15163, "n_datasets": 1,
            "experiment": "volume"},
    "B1": {"name": "pretrain_B1_two_dataset", "group": "DATA_SCALING_DIVERSITY",
            "label": "B1: Klinzing+Shirazi (2ds)", "eff_data": 34647, "n_datasets": 2,
            "experiment": "diversity"},
    "B2": {"name": "pretrain_B2_three_dataset", "group": "DATA_SCALING_DIVERSITY",
            "label": "B2: +Pavlov (3ds)", "eff_data": 37134, "n_datasets": 3,
            "experiment": "diversity"},
    "B3": {"name": "pretrain_B3_four_dataset", "group": "DATA_SCALING_DIVERSITY",
            "label": "B3: +Getzmann (4ds)", "eff_data": 48001, "n_datasets": 4,
            "experiment": "diversity"},
    "C1": {"name": "pretrain_C1_headband_only", "group": "DATA_SCALING_CONTROLS",
            "label": "C1: Headband only (~6ch)", "eff_data": 7292, "n_datasets": 1,
            "experiment": "controls"},
    "C2": {"name": "pretrain_C2_volume_matched", "group": "DATA_SCALING_CONTROLS",
            "label": "C2: 3ds vol-matched", "eff_data": 19580, "n_datasets": 3,
            "experiment": "controls"},
    "D1": {"name": "pretrain_D1_kochi_only", "group": "DATA_SCALING_PARADIGM",
            "label": "D1: Kochi only", "eff_data": 2565, "n_datasets": 1,
            "experiment": "paradigm"},
    "D2": {"name": "pretrain_D2_klinzing_kochi", "group": "DATA_SCALING_PARADIGM",
            "label": "D2: Klinzing+Kochi", "eff_data": 22049, "n_datasets": 2,
            "experiment": "paradigm"},
    "D3": {"name": "pretrain_D3_klinzing_shirazi_kochi", "group": "DATA_SCALING_PARADIGM",
            "label": "D3: Klinzing+Shirazi+Kochi", "eff_data": 37211, "n_datasets": 3,
            "experiment": "paradigm"},
    "E1": {"name": "pretrain_E1_all_datasets", "group": "DATA_SCALING_MAXDATA",
            "label": "E1: All 5 datasets", "eff_data": 50566, "n_datasets": 5,
            "experiment": "maxdata"},
}
...
def fetch_pretrain_losses(api: wandb.Api) -> pd.DataFrame:
    ...
    for run_id, info in PRETRAIN_RUNS.items():
        print(f"  Fetching pretrain {run_id}: {info['name']} (group={info['group']})...")
        runs = api.runs(
        ...
        train_history = run.history(keys=["train/loss"], samples=50000, pandas=True)

        for _, row in val_history.iterrows():
            records.append({
                "run_id": run_id, "run_name": info["name"], "label": info["label"],
                "experiment": info["experiment"],
                "step": row.get("_step"), "val_loss": row.get("val/loss"), "train_loss": None,
            })
        ...
        for _, row in train_history.iterrows():
            records.append({
                "run_id": run_id, "run_name": info["name"], "label": info["label"],
                "experiment": info["experiment"],
                "step": row.get("_step"), "val_loss": None, "train_loss": row.get("train/loss"),
            })

        best_val = val_history["val/loss"].dropna().min() if "val/loss" in val_history.columns and len(val_history) > 0 else None
        total_steps = run.summary.get("_step", None)
        if best_val is not None:
            final_val = val_history["val/loss"].dropna().iloc[-1]
            print(f"    state={run.state}, steps={total_steps}, best_val_loss={best_val:.4f}, final_val_loss={final_val:.4f}")
        else:
            print(f"    state={run.state}, steps={total_steps}, no val/loss data")
...
def fetch_downstream_results(api: wandb.Api) -> pd.DataFrame:
    ...
    for (task, mode), group in DOWNSTREAM_GROUPS.items():
        ...
        runs = api.runs(f"{entity}/{DOWNSTREAM_PROJECT}", filters={"group": group})
        run_list = list(runs)
...
def summarize_downstream(df: pd.DataFrame) -> pd.DataFrame:
    ...
    summary = (
        df.groupby(["task", "mode", "pretrain_run_id", "pretrain_label", "experiment", "eff_data", "n_datasets"])
        ["best_f1"]
        .agg(["mean", "std", "count"])
        .reset_index()
    )
...
def plot_pretrain_loss_curves(pretrain_df: pd.DataFrame) -> list[Path]:
    ...
    for ax_idx, (exp_key, (exp_title, run_ids)) in enumerate(experiments.items()):
        ax = axes[ax_idx]
        for rid in run_ids:
            ...
            ax.plot(subset["step"], subset["val_loss"],
                    label=rid, color=RUN_COLORS[rid], linewidth=1.5)
    ...
    fig.suptitle("Pretraining Validation Loss Curves — All Experiments",
    ...
    for rid in PRETRAIN_RUNS:
        ...
        ax.plot(subset["step"], subset["val_loss"],
                ...
                color=RUN_COLORS[rid], linewidth=1.2)

    ax.set_xlabel("Step", fontsize=11)
    ax.set_ylabel("Val Loss (MAE reconstruction)", fontsize=11)
    ax.set_title("Pretraining Validation Loss — All Runs Overlaid",
...
def plot_pretrain_final_loss_bar(pretrain_df: pd.DataFrame) -> Path:
    ...
    for bar, loss in zip(bars, losses):
        ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.001,
                f"{loss:.4f}", ha="center", va="bottom", fontsize=8, fontweight="bold")

    ax.set_xticks(range(len(run_ids)))
    ax.set_xticklabels([f"{r}\n({PRETRAIN_RUNS[r]['eff_data']//1000}k ch·h)" for r in run_ids],
                       fontsize=8)
    ax.set_ylabel("Final Val Loss", fontsize=11)
    ax.set_title("Pretraining Final Validation Loss — All Runs\n(lower = better reconstruction)",
...
def plot_downstream_grand_comparison(summary_df: pd.DataFrame, mode: str) -> Path:
    ...
    ordered_runs = [r for r in PRETRAIN_RUNS if r in sub["pretrain_run_id"].values]
    if not ordered_runs:
        return None
    ...
    for ax_idx, task in enumerate(TASKS):
        ...
        bars = ax.bar(x, means, 0.7, yerr=stds, capsize=3,
                      color=colors, edgecolor="white", linewidth=0.5,
                      error_kw=dict(lw=1))

        if mode == "finetune":
            _, baseline_val = BEST_BASELINE[task]
            ax.axhline(y=baseline_val, color="black", linestyle="--", linewidth=1.5,
                       label=f"Best baseline ({baseline_val:.3f})")
            _, poyo_val = BEST_POYO_BASELINE[task]
            ax.axhline(y=poyo_val, color="gray", linestyle=":", linewidth=1.2,
    ...
    fig.suptitle(f"Downstream {mode_label} — All Pretraining Configurations",
                 fontsize=13, fontweight="bold", y=1.02)
    plt.tight_layout()
...
def plot_delta_heatmap(summary_df: pd.DataFrame) -> Path:
    ...
    vmax = max(0.03, np.nanmax(np.abs(delta_matrix))) if not np.all(np.isnan(delta_matrix)) else 0.03
    ...
    ax.set_yticklabels([f"{r} ({PRETRAIN_RUNS[r]['eff_data']//1000}k)" for r in ordered_runs],
                       fontsize=9)
    plt.colorbar(im, ax=ax, shrink=0.8, label="ΔF1 vs best baseline")

    ax.set_title("Finetuning Transfer Gain vs Best Baseline\n(all pretraining configs)",
                 fontsize=12, fontweight="bold")
    plt.tight_layout()
...
def plot_scaling_curves(summary_df: pd.DataFrame) -> Path:
    ...
    for ax_idx, task in enumerate(TASKS):
        ...
        for exp_name, exp_color in EXPERIMENT_COLORS.items():
            exp_data = task_data[task_data["experiment"] == exp_name].sort_values("eff_data")
            if exp_data.empty:
                continue
            ax.errorbar(exp_data["eff_data"], exp_data["mean"],
                        yerr=exp_data["std"].fillna(0),
                        marker="o", color=exp_color, linewidth=1.5, capsize=4,
                        label=exp_name.capitalize(), markersize=6)
            for _, row in exp_data.iterrows():
                ax.annotate(row["pretrain_run_id"],
                            (row["eff_data"], row["mean"]),
                            textcoords="offset points", xytext=(5, 5),
                            fontsize=7, color=exp_color)

        _, baseline_val = BEST_BASELINE[task]
        ax.axhline(y=baseline_val, color="black", linestyle="--", linewidth=1.2,
                   label=f"Best baseline ({baseline_val:.3f})")
    ...
    fig.suptitle("Downstream Finetuning F1 vs Effective Pretraining Data",
                 fontsize=13, fontweight="bold", y=1.02)
    plt.tight_layout()
...
def plot_diversity_vs_volume(summary_df: pd.DataFrame) -> Path:
    ...
    fig, axes = plt.subplots(len(key_comparisons), 3, figsize=(18, 4 * len(key_comparisons)))

    for row_idx, (comp_title, run_ids) in enumerate(key_comparisons.items()):
        for col_idx, task in enumerate(TASKS):
            ax = axes[row_idx, col_idx] if len(key_comparisons) > 1 else axes[col_idx]
            task_data = sub[sub["task"] == task]
            ...
            for rid in run_ids:
                ...
                if not row.empty:
                    ...
                    stds.append(0)
                colors.append(RUN_COLORS[rid])
                x_labels.append(f"{rid}\n({PRETRAIN_RUNS[rid]['eff_data']//1000}k)")

            x = np.arange(len(run_ids))
            bars = ax.bar(x, means, 0.5, yerr=stds, capsize=4,
                          color=colors, edgecolor="white", linewidth=0.5,
                          error_kw=dict(lw=1))
            ...
            if means:
                ...
                if valid:
                    ax.set_ylim(min(min(valid), baseline_val) - 0.03,
                                max(max(valid), baseline_val) + 0.03)
            ax.legend(fontsize=7, loc="lower right")

    fig.suptitle("Key Comparisons: Diversity vs Volume vs Controls",
...
def plot_linear_probe_heatmap(summary_df: pd.DataFrame) -> Path:
    ...
    for i in range(len(ordered_runs)):
        for j in range(len(TASKS)):
            ...
            if not np.isnan(val):
                ax.text(j, i, f"{val:.3f}", ha="center", va="center",
                        fontsize=9, fontweight="bold",
                        color="white" if val > (np.nanmax(f1_matrix) + np.nanmin(f1_matrix)) / 2 else "black")
    ...
    ax.set_title("Linear Probe F1 — Representation Quality\n(frozen backbone, all pretraining configs)",
                 fontsize=12, fontweight="bold")
    plt.tight_layout()
...
def plot_per_experiment_downstream(summary_df: pd.DataFrame, experiment: str,
                                   run_ids: list[str], title: str) -> Path:
    ...
    for mode_idx, mode in enumerate(["finetune", "linear_probe"]):
        ...
        for col_idx, task in enumerate(TASKS):
            ...
            available_runs = [r for r in run_ids if r in task_data["pretrain_run_id"].values]
            ...
            ax.set_xticks(x)
            ax.set_xticklabels([f"{r}\n({PRETRAIN_RUNS[r]['eff_data']//1000}k)" for r in available_runs],
                               fontsize=9)
            if col_idx == 0:
...
def print_pretrain_summary(pretrain_df: pd.DataFrame) -> None:
    ...
    print("=" * 85)
    print(f"  {'Run':<5} {'Label':<35} {'Eff Data':>10} {'Final Val Loss':>15} {'Best Val Loss':>15}")
    ...
    for rid in PRETRAIN_RUNS:
        ...
        subset = pretrain_df[pretrain_df["run_id"] == rid].dropna(subset=["val_loss"])
        if subset.empty:
            print(f"  {rid:<5} {info['label']:<35} {info['eff_data']:>10,} {'N/A':>15} {'N/A':>15}")
            continue
        subset = subset.sort_values("step")
        final_loss = subset["val_loss"].iloc[-1]
        best_loss = subset["val_loss"].min()
        print(f"  {rid:<5} {info['label']:<35} {info['eff_data']:>10,} {final_loss:>15.4f} {best_loss:>15.4f}")
...
def print_downstream_summary(summary_df: pd.DataFrame) -> None:
    ...
    for mode in ["finetune", "linear_probe"]:
        ...
        for task in TASKS:
            task_data = sub[sub["task"] == task].sort_values("mean", ascending=False)
            if task_data.empty:
                continue

            baseline_name, baseline_val = BEST_BASELINE[task]
            print(f"\n  {task} (baseline: {baseline_name} = {baseline_val:.3f})")
            print(f"  {'Run':<5} {'Label':<35} {'Mean F1':>8} {'± Std':>8} {'N':>3} {'Δ Baseline':>12}")
            print(f"  {'-'*5} {'-'*35} {'-'*8} {'-'*8} {'-'*3} {'-'*12}")
            ...
            for _, row in task_data.iterrows():
                ...
                delta_str = f"{delta:>+12.3f}" if delta is not None else f"{'':>12}"
                print(f"  {rid:<5} {label:<35} {mean_f1:>8.3f} {std_f1:>8.3f} {n:>3} {delta_str}")
    ...
    experiments = {
        "volume": {
            ...
            "comparisons": [
                ...
                ("A2 vs A3", "Similar effective data, Shirazi has 129ch vs Klinzing ~10ch"),
            ],
        },
        "diversity": {
        ...
        "controls": {
            ...
            "comparisons": [
                ...
                ("C2 vs A2", "3-source vol-matched vs single-source at ~19.5k ch·h"),
            ],
        },
        "paradigm": {
    ...
    for exp_key, exp_info in experiments.items():
        ...
        for task in TASKS:
            ...
            for rid in exp_info["runs"]:
                ...
                if not row.empty:
                    m = row["mean"].values[0]
                    s = row["std"].values[0] if not np.isnan(row["std"].values[0]) else 0
                    d = m - baseline_val
                    print(f"    {rid}: {m:.3f} ± {s:.3f} (Δ baseline: {d:+.3f})")
                else:
                    print(f"    {rid}: N/A")
        ...
        for comp_label, comp_desc in exp_info["comparisons"]:
            ...
            run_ids = [r.strip() for r in comp_label.replace("→", "vs").split("vs")]
            for task in TASKS:
                vals = {}
                for rid in run_ids:
                    row = ft[(ft["task"] == task) & (ft["pretrain_run_id"] == rid)]
                ...
                if len(vals) == 2:
                    ...
                    print(f"      {task}: {rids[0]}={vals[rids[0]]:.3f}, {rids[1]}={vals[rids[1]]:.3f}, Δ={diff:+.3f}")
...
def main():
    ...
    if not valid.empty:
        print(f"\n  Per-fold detail:")
        for _, row in valid.sort_values(["task", "mode", "pretrain_run_id", "fold"]).iterrows():
            print(
    ...
    if not summary_df.empty:
        ...
        per_exp = {
            ...
            "diversity": (["B1", "B2", "B3"], "Diversity Scaling: B1 → B2 → B3"),
            "controls": (["C1", "C2"], "Controls: C1 (Headband) & C2 (Vol-Matched)"),
            "paradigm": (["D1", "D2", "D3"], "Paradigm Diversity: D1 → D2 → D3"),
            "maxdata": (["E1"], "Maximum Data: E1 (All 5 Datasets)"),
        }
        for exp_key, (run_ids, title) in per_exp.items():
            p = plot_per_experiment_downstream(summary_df, exp_key, run_ids, title)
            if p:
                generated.append(p)

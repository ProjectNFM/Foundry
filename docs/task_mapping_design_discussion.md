...
The task system is a declarative pipeline that connects YAML task definitions → dataset registration → target extraction at tokenization → readout heads → losses → metrics → confusion matrices.
...
│                                                                         │
│  Dataset class (e.g. KempSleepEDF2013)                                  │
│       └──► AVAILABLE_TASKS = {"sleep_stage_5class": TaskConfig(...)}     │
│       └──► TASK_TO_READOUT = {"sleep_stage": ["sleep_stage_5class"]}     │
│                                                                         │
...
| `TaskMixin` | `foundry/data/datasets/mixins.py` | Dataset mixin: task registry + `get_tasks_for_experiment()` |
| `filter_intervals_by_mapping` | `foundry/tasks/classification_mapping.py` | Removes sampling intervals for excluded classes |

### Key Invariants

1. **Task configs are dataset-owned** — experiments select tasks via `data.task_type`, not by declaring configs directly.

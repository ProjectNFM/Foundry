...
\textbf{Setup:} A new \texttt{per\_channel\_resample\_cnn\_64f}
configuration mirrors the CWT+CNN's convolutional stack (64 filters,
kernel 9, 2 layers) but operates directly on the resampled waveform
instead of a CWT scalogram. Both tokenizers are trained at 200\,Hz
target token rate with the same hyperparameters (LR=3e-4, WD=0.007,
embed\_dim=256, concat fusion), 2 folds. The original CWT and CNN
(12 filters) from the TOKEN\_RATE\_SWEEP are included for context.
...
\begin{table}[H]
      ...
      \begin{tabular}{lrrr}
            ...
            \bottomrule
      \end{tabular}
      \caption{Tokenizer configurations in the parameter-matched
            comparison. CNN (64f) and CWT+CNN (64f) are within $\sim$20\%
            of each other in parameter count, with the remaining gap due
            to the CWT's learnable parameters and higher input
...
\begin{figure}[H]
      ...
      \caption{Parameter-matched comparison at 200\,Hz. Left pair:
            original small tokenizers from the TOKEN\_RATE\_SWEEP.
            Right pair: parameter-matched variants with 64 conv filters.
            CWT+CNN retains its advantage even when the CNN has
            comparable capacity.}
...
\textbf{Observations:}
\begin{itemize}[nosep]
      \item CWT+CNN (64f) achieves a mean AUROC of $\sim$0.898,
            outperforming the parameter-matched CNN (64f) at
            $\sim$0.887---a gap of $\sim$1.1\%.
      \item For context, the original CWT ($\sim$0.889) and CNN 12f
            ($\sim$0.884) from the token-rate sweep
            (Section~\ref{sec:token-rate-scaling}) bracket the CNN
            (64f) result, indicating that simply scaling up CNN
            filters from 12 to 64 ($\sim$13$\times$ more parameters)
            yields only a marginal improvement ($\sim$0.3\%).
      \item The CWT+CNN's advantage over both CNN variants confirms an
            architectural benefit: the CWT scalogram provides a
            structured input that the convolutional stack can exploit
            more effectively than a raw waveform, even when the CNN
            has comparable capacity.
\end{itemize}

\textbf{Caveats:} The parameter counts are not perfectly matched
(59{,}858 vs.\ 50{,}048, a $\sim$20\% gap), with the difference
stemming from the CWT's learnable parameters and the higher
input dimensionality to the first convolutional layer (18 channels
from the scalogram vs.\ 1 for the plain CNN). A tighter match
(e.g., CWT+CNN with 12 filters) could further isolate the
architectural contribution.

\subsubsection*{Joint Interpretation}

The CWT+CNN architecture successfully combines the CWT's
sampling-rate-invariant frequency decomposition with the CNN's ability
to learn cross-frequency and local-temporal features. The convolutional
stack can learn to combine magnitude and phase information across
frequency bins in ways that a single linear projection cannot. The
parameter-matched comparison confirms that this advantage is at least
partly architectural ($\sim$1.1\% over a 64-filter CNN), though the
gap narrowed from $\sim$2\% (with a 15$\times$ parameter difference)
to $\sim$1.1\% (with roughly matched parameters), indicating that
increased capacity also contributed to the original advantage.
...
CWT's structured representation. The parameter-matched comparison
(Section~\ref{sec:param-match}) confirms that this advantage is
at least partly architectural: CWT+CNN outperforms a CNN with
comparable capacity (64 filters, $\sim$50k params) by $\sim$1.1\%
AUROC. However, the gap narrowed from $\sim$2\% (with a
15$\times$ parameter difference) to $\sim$1.1\% (with roughly
matched parameters), indicating that increased capacity also
contributed to the original advantage.
...
\begin{itemize}[nosep]
      ...
      \item \textbf{CWT+CNN parameter matching} (resolved): The
            ...
            (Section~\ref{sec:param-match}) confirms that CWT+CNN's
            advantage is at least partly architectural ($\sim$1.1\%
            over a 64-filter CNN). A follow-up with CWT+CNN at 12
            filters would test whether the hybrid still wins at lower
            capacity.

...
def test_warmup_hold_cosine_lr_lambda_phases():
    from foundry.training import FoundryModule
    ...
    assert module.lr_lambda(0) == pytest.approx(0.0)
    assert module.lr_lambda(2) == pytest.approx(0.5)
    assert module.lr_lambda(4) == pytest.approx(1.0)  # start of hold
    assert module.lr_lambda(5) == pytest.approx(1.0)  # still hold
    # Mid-decay: progress=0.5 → cos(pi/2)=0 → factor = 0.1 + 0.9*0.5 = 0.55
    assert module.lr_lambda(8) == pytest.approx(0.55)
    assert module.lr_lambda(10) == pytest.approx(0.1)
    assert module.lr_lambda(100) == pytest.approx(0.1)


def test_cosine_anneal_decay_oscillates_then_decays():
    """Test that cosine_anneal_decay oscillates in phase 1, then decays in phase 2."""
    import math

    from foundry.training import FoundryModule

    cfg = TaskConfig.from_yaml(TASKS_CONFIG_DIR / "neurosoft_on_vs_off.yaml")
    # Use a simple T_0 = 100 for testing. With max_epochs not set (trainer=None),
    # this will default to 100.
    module = FoundryModule(
        model=_StubTaskModel({cfg.name: cfg}),
        scheduler="cosine_anneal_decay",
        cosine_steps=100,
        decay_steps=100,
        min_lr_factor=0.1,
    )

    # Phase 1: Oscillation via 0.5 * (1 + cos(2π * t / T_0))
    # At t=0: cos(0) = 1 → factor = 1.0
    assert module.lr_lambda(0) == pytest.approx(1.0)
    
    # At t=T_0/4=25: cos(π/2) = 0 → factor = 0.5
    assert module.lr_lambda(25) == pytest.approx(0.5)
    
    # At t=T_0/2=50: cos(π) = -1 → factor = 0.0
    assert module.lr_lambda(50) == pytest.approx(0.0)
    
    # At t=3*T_0/4=75: cos(3π/2) = 0 → factor = 0.5
    assert module.lr_lambda(75) == pytest.approx(0.5)
    
    # At t=T_0=100: still in phase 1, cos(2π) = 1 → factor = 1.0
    # But we're at the boundary, so just past it enters phase 2
    
    # Phase 2: Start of decay at step 100
    # progress = 0 → factor = 1.0
    assert module.lr_lambda(100) == pytest.approx(1.0)
    
    # Mid-decay at step 150 (progress = 0.5)
    # cos(π * 0.5) = 0 → factor = 0.1 + 0.9 * 0.5 = 0.55
    assert module.lr_lambda(150) == pytest.approx(0.55)
    
    # End of decay at step 200
    assert module.lr_lambda(200) == pytest.approx(0.1)
    
    # Hold at min_lr_factor
    assert module.lr_lambda(300) == pytest.approx(0.1)


def test_cosine_anneal_decay_with_zero_cosine_steps():
    """When cosine_steps=0, skip oscillation and go straight to decay."""
    from foundry.training import FoundryModule
    ...
    # No oscillation, go straight into decay
    assert module.lr_lambda(0) == pytest.approx(1.0)
    assert module.lr_lambda(50) == pytest.approx(0.55)
    assert module.lr_lambda(100) == pytest.approx(0.1)


def test_cosine_anneal_only_with_no_decay():
    """When decay_steps=0, only oscillate then hold at 1.0."""
    from foundry.training import FoundryModule
    ...
    # Past oscillation: hold at min_lr_factor
    assert module.lr_lambda(100) == pytest.approx(0.1)

def test_configure_optimizers_uses_lambda_lr():
    from foundry.training import FoundryModule
